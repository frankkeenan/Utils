...
#	tr|A-Z|a-z|;
    ...
    # add end tags to empty elements ...
	s|<([^/> ]+)( [^>]*)?/>|<$1$2></$1>|g;
	...
	{
	    $_ = &get_more($_);
		$_ =~ s|<([^/> ]+)( [^>]*)?/>|<$1$2></$1>|g; # add end tags to empties ...
	}
	$cp = $_;
	$cp =~ s|<nl */>|\n|g;
	if ($ct++ < $opt_l)
	...
	foreach $tag (@TAGS)
	{
		...
		if ($tag =~ m|^<([^/ >]+)( ([^>]*))?>([^<]+)$|)
		{
			$tag =~ s|^<([^/ >]+)( ([^>]*))?>([^<]+)$|$&</$1>|; # add missing end tag to $tag ...
		}
#		$tag =~ s|<|\{|g;
#		$tag =~ s|>|\}|g;
		$tagname =~ s|[ >].*||o;
		...
		unless ($Found{$tagname})
		{
			...
			if ($tag =~ m|^<([^>]*)>(</([^>]*)>)?$|)
			{
				$TAG_EG{$tagname} = sprintf("<entry><h>$tagname</h><z> as container inside entry </z><ei>$h</ei></entry>"); 
			}
			else
			{

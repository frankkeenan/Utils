#!/usr/local/bin/perl
#
# $Id: unicode_module.pl,v 1.1 2013/07/08 11:09:47 keenanf Exp keenanf $
# $Log: unicode_module.pl,v $
# Revision 1.1  2013/07/08 11:09:47  keenanf
# Initial revision
#
...
# $res = &unicode2ents
# $res = &lose_non_letter_ents
# $res = &get_unicode_sort_key_ex
# $res = &get_unicode_sort_key
# $res = &lose_ents
# $res = &lose_ent_spaces
# $res = &phon_ents($_);
#&load_phon_unicode
...
sub phon_ents
{
    ...
    my $otag, $ctag;
    my @CHARS;
    $e =~ s|(<phon-gb[^>]*)/>|$1></phon-gb>|gi;
    $e =~ s|(<phon-us[^>]*)/>|$1></phon-us>|gi;
    $e =~ s|(<i [^>]*)/>|$1></i>|gi;
    $e =~ s|(<y [^>]*)/>|$1></y>|gi;
	...
	if ($bit =~ s|&fk;||gio)
	{
	    ...
	    $bit =~ s|</?wb *>||gio;
#	    if ($bit =~ m|^(.*>)(.*?)(</[iy]>)|io)
	    if ($bit =~ m/^(.*>)(.*?)(<\/(i|y|phon-gb|phon-us|eph|phon)>)/io)
	    {
		$otag = $1;
		$phon = $2;
		$ctag = $3;		
